...
/* ── Shared: build & save an Order from the customer's current cart ──
   Used by both /verify (normal path) and the webhook (recovery path
   when the client disappeared before /verify could run). Idempotent:
   safe to call twice for the same razorpayOrderId. */
